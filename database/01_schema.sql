...
CREATE TABLE `event_meetings` (
  ...
  `event_id`      BIGINT UNSIGNED NOT NULL,
  `title`         VARCHAR(200) NOT NULL,
  `reason`        TEXT NULL COMMENT 'Lý do / nội dung trọng tâm buổi họp',
  `status`        ENUM('scheduled','cancelled') NOT NULL DEFAULT 'scheduled',
  `cancelled_at`  DATETIME(6) NULL COMMENT 'Khi hủy buổi họp',
  `meeting_type`  ENUM('pre_event','in_event') NOT NULL DEFAULT 'pre_event',
  `start_at`      DATETIME(6) NOT NULL,
  `end_at`        DATETIME(6) NOT NULL,
  `actual_end_at` DATETIME(6) NULL COMMENT 'Khi quản lý nhấn kết sớm; null = kết thúc theo end_at',
  ...
  KEY `ix_meet_event_status` (`event_id`, `status`),
